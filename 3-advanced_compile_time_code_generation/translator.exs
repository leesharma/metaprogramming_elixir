defmodule Translator do
  ...
  defmacro __before_compile__(env) do
    IO.inspect Module.get_attribute(env.module, :locales)
  ...
  def compile(translations) do
    ...
    final_ast = quote do
      def t(locale, path, bindings \\ []) # header
      unquote(translations_ast)
      def t(_locale, _path, _bindings), do: {:error, :no_translation} # fallback
    end
  ...
  defp interpolate(string) do
    ...
    |> Enum.reduce("", &append_segment/2)
  end

  defp append_segment(<<"%{" <> rest>>, acc) do
  ...
  defp append_segment(segment, acc) do
